...
# Default timeout for health check commands.
DEFAULT_HEALTH_CHECK_TIMEOUT = 5.0  # seconds


async def health_check(computer: Computer, *, timeout: float = DEFAULT_HEALTH_CHECK_TIMEOUT) -> bool:
    """Check whether a computer is responsive.

    Runs ``echo ok`` inside the computer with a short timeout.  Returns
    ``True`` if the command succeeds (exit code 0), ``False`` otherwise.

    This is a protocol-level utility — individual ``Computer``
    implementations may provide faster checks (e.g. API status calls for
    remote sandboxes), but the default works universally.

    Args:
        computer: The computer to probe.
        timeout: Maximum seconds to wait for the command.

    Returns:
        ``True`` if the computer responded successfully.
    """
    try:
        result = await computer.run("echo ok", timeout=timeout)
