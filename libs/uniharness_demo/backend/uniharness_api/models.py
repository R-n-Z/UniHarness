...
class ErrorResponse(BaseModel):
    """Structured error response for API and SSE error events.

    All errors flowing through the API layer are normalised to this shape
    so clients can rely on ``error_code`` rather than parsing message strings.
    """

    error_code: str = Field(..., description="Machine-readable error code (see ErrorCode enum)")
    message: str = Field(..., description="Human-readable error description")
    retryable: bool = Field(False, description="Whether the client may retry this operation")
    trace_id: str | None = Field(None, description="Request trace ID for correlating logs")
