...
from uniharness.exceptions import ErrorCode, UniHarnessError, VMMountConflictError

from uniharness_api.agent_manager import agent_manager
from uniharness_api.config import load_config
from uniharness_api.models import ErrorResponse, MessageRequest
...
async def send_message(conversation_id: str, body: MessageRequest) -> StreamingResponse:
    ...
    async def event_stream():  # noqa: C901
        ...
        try:
            ...
            if isinstance(exc, UniHarnessError):
                err = ErrorResponse(
                    error_code=exc.error_code.value,
                    message=str(exc),
                    retryable=exc.error_code.is_retryable,
                )
            else:
                err = ErrorResponse(
