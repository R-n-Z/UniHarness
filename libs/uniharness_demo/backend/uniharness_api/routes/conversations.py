...
async def list_conversations() -> list[dict]:
    ...
    return [c.to_detail() for c in await store.list_all()]
...
async def create_conversation(body: ConversationCreateRequest | None = None) -> dict:
    ...
    conv = await store.create(title=title, model_id=model_id, mode=mode, working_dir=working_dir)
    ...
    if session_id:
        warm = await session_store.claim(session_id)
        if warm is not None:
...
async def update_conversation(conversation_id: str, body: ConversationUpdateRequest) -> dict:
    ...
    if body.model_id is not None:
        await store.update_model_id(conversation_id, body.model_id)
